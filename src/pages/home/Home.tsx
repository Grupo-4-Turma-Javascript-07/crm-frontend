...
export default function Home() {
  ...
  const addTask = () => {
    ...
    if (modalType === 'afazeres') {
      const updatedTasks = editingIndex !== null ? [...(user.afazeres ?? [])] : [...(user.afazeres ?? []), newTask];
      if (editingIndex !== null) updatedTasks[editingIndex] = newTask;
      setUser({ ...user, afazeres: updatedTasks });
    } else if (modalType === 'solicitacoes') {
      const updatedTasks = editingIndex !== null ? [...(user.solicitacoes ?? [])] : [...(user.solicitacoes ?? []), newTask];
      if (editingIndex !== null) updatedTasks[editingIndex] = newTask;
      setUser({ ...user, solicitacoes: updatedTasks });
    } else if (modalType === 'atividades') {
      const updatedTasks = editingIndex !== null ? [...(user.atividades ?? [])] : [...(user.atividades ?? []), newTask];
      if (editingIndex !== null) updatedTasks[editingIndex] = newTask;
      setUser({ ...user, atividades: updatedTasks });
    }
  ...
  const deleteTask = (index: number) => {
    if (modalType === 'afazeres') setUser({ ...user, afazeres: user.afazeres.filter((_, i) => i !== index) });
    else if (modalType === 'solicitacoes') setUser({ ...user, solicitacoes: user.solicitacoes.filter((_, i) => i !== index) });
    else if (modalType === 'atividades') setUser({ ...user, atividades: (user.atividades ?? []).filter((_, i) => i !== index) });
  };

  const editTask = (index: number) => {
    if (modalType === 'afazeres') setNewTask(user.afazeres[index]);
    else if (modalType === 'solicitacoes') setNewTask(user.solicitacoes[index]);
    else if (modalType === 'atividades') setNewTask(user.atividades?.[index] ?? '');
    ...
    <aside className={`bg-[#191a1f] p-6 flex flex-col justify-between transition-all duration-300 ${menuOpen ? 'w-64' : 'w-16'} pt-24`}>
      <div>
        ...
        <nav className="flex flex-col gap-4 text-gray-300 font-semibold">
          ...
          <button
            className="flex items-center gap-3 hover:text-purple-500 font-bold focus:outline-none"
      ...
      <div className="flex-1 flex flex-col">
        <header className="fixed top-0 left-0 w-full flex items-center justify-between bg-[#191a1f] p-4 z-[999]">
          <div className="flex items-center gap-3">
            <img src="https://cdn.discordapp.com/attachments/1369437328153645093/1404901686274691082/CRM4ULOGO.png?ex=689d88e7&is=689c3767&hm=d27ac9332332ed403c4b9f44d32e632a1e976891f71b60f99ba55ee406630dfb" alt="Logo" className="h-15 w-auto cursor-pointer" />
          </div>
            ...
            <section className="bg-[#2a2a2a] rounded-md">
              ...
              <div className="p-4 flex flex-col gap-3 justify-center items-center">
                <div className="flex gap-2 w-[1000px]">
                  ...
                  <button
                    onClick={addTask}
                    className="flex-1 inline-flex justify-center items-center py-3 px-6 border border-transparent shadow-lg text-sm font-bold rounded-lg text-white
                               bg-gradient-to-r from-purple-500 to-pink-400 hover:from-purple-400 hover:to-pink-300
                  ...
                  {user.afazeres.map((task, index) => (
                    <li key={index} className="flex justify-between items-center bg-[#1b1b1b] p-2 rounded-md w-[800px]">
                      ...
                      <div className="flex gap-2">
                        <FiEdit2 className="text-yellow-200 hover:text-yellow-500 cursor-pointer" onClick={() => editTask(index)} />
                        <FiTrash2 className="text-red-300 hover:text-red-500 cursor-pointer" onClick={() => deleteTask(index)} />
                      </div>
                    </li>
                  ))}
                </ul>
            ...
            {(modalType === 'afazeres' || modalType === 'solicitacoes' || modalType === 'atividades') && (
              <div className="mb-4 flex gap-2">
                <input
                  ...
                  placeholder={`Nova ${modalType === 'afazeres' ? 'tarefa' : modalType === 'atividades' ? 'atividade' : 'solicitação'}...`}
